...
class InfluxWrapper:
    ...
    :param str token: Token di accesso per il database; di fatto l'utente con il quale ci stiamo loggando.
    '''
    ...
    def write(self, db_name: str, data: list[dict]):
        '''
        Questa funzione si occupa di interpretare il risultato di Responde dal server Prometheus.

        :param str db_name: Nome del database dove salvare i dati
        :param list[dict]: I dati grezzi che devono essere inseriti
        '''
        with InfluxDBClient3(host=self.url, token=self.token, org='uninuvola', database='uninuvola') as client:
            for result in data:
                ...
                if 'values' in data_keys:
                    ...
                    ''' In questa condizione non so cosa fare, nei miei test non è mai successo '''
                    self.log.error("Never have the case data_keys DON'T CONTAINS [values or value], I DONT KNOW WHAT TO DO")
