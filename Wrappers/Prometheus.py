...
class PrometheusWrapper:
    ...
    :param bool tls: [Opzionale] Booleano che informa il sistema se le chiamate
                     sono fatte tramite tls, se i cerficati non sono validi
                     la chiamata fallirà.
    """
    ...
    def __init__(self, url: str, tls: bool = True):
        ...
        if not tls:
            """In caso l'url non avesse un certificato valido disabilito
            i warning perchè inquinano l'output dei logs
            """
            disable_warnings()
    ...
    def query(
        ...
        if start is None and end is None and step is None:
            ...
            self.log.debug(f"Params: {params}")
            prometheus_reponse = get(
                self.query_range_url, params=params, verify=self.tls
            )
        else:
