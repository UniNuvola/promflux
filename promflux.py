...
def main(arguments: Namespace):
    ...
    for rule in rules:
        query_result = prometheus.query(query=rule.query, start=rule.start, end=rule.end, step=rule.step)
        ...
        if data['resultType'] != 'vector':
            log.warning(f"Prometheus query retuned: resultType == {data['resultType']} -> NOT TESTED!! BE AWARE!!")
...
if __name__ == "__main__":
    ...
    server_args.add_argument('prometheus',
                             nargs='?',
                             help='Addess for the Prometheus server')
    
    server_args.add_argument('influx',
                             nargs='?',
    ...
    settings.add_argument('-t',
                          '--token',
                          type=str,
                          help="Influx token"
                          )

    settings.add_argument('-f',
                          '--file',
                          type=str,
